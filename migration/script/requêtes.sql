...
SELECT scientific_name,common_name,name
FROM species
JOIN family ON family_id = family.id; -- c'est la condition de jointure (pour rassembler les résultats)
...
SELECT field.name,array_agg(row.label)
FROM row
JOIN field ON field.id = field_id
JOIN variety ON variety.id = row.variety_id
WHERE bitterness = 5
...
select family.name from family where family.id not in (
select distinct family_id from species where species.id in (
select species.id from species join variety on species_id = species.id group by species.id having avg(juiciness)>2.5));
...
select distinct field.name from field join row on row.field_id = field.id where row.id in (
select row.id from row where variety_id in (	
select variety.id from variety join species on variety.species_id = species.id where species.id in (	
select species.id from species join family on family.id = species.family_id where species.family_id in (	
select family.id from family where name = 'mandarine'))));
